...
def get(**kwargs):
    ...
    print Idc()
    print getattr(Idc,tmp_order_by[0])
    print getattr(getattr(Idc,tmp_order_by[0]), tmp_order_by[1])()
    data = db.session.query(Idc).filter_by(**where).order_by(getattr(getattr(Idc,tmp_order_by[0]), tmp_order_by[1])()).limit(limit).all()
    db.session.close()
    ret = []
    for obj in data:
