...
def create(**kwargs):
    ...
    for field in kwargs.keys():
        if not hasattr(Manufacturers,field):
            current_app.logger.warning("参数错误，{},不再manufacturers这张表中".format(field))
...
def get(**kwargs):
    ...
    if not hasattr(Manufacturers, tmp_order_by[0].lower()):
    ...
    print Manufacturers()
    print getattr(Manufacturers,tmp_order_by[0])
    print getattr(getattr(Manufacturers,tmp_order_by[0]), tmp_order_by[1])()
    data = db.session.query(Manufacturers).filter_by(**where).order_by(getattr(getattr(Manufacturers,tmp_order_by[0]), tmp_order_by[1])()).limit(limit).all()
    db.session.close()
    ret = []
    for obj in data:
