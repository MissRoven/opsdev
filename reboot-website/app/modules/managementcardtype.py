...
def create(**kwargs):
    ...
    # 3  插入到数据库
    manufacturers = ManagementCard(**kwargs)
...
def get(**kwargs):
    ...
    # 4. 验证order_by
    tmp_order_by = check_order_by(ManagementCard, order_by)
    # 5. 验证limit
    check_limit(limit)

    #查询
    data = db.session.query(ManagementCard).filter_by(**where).order_by(getattr(getattr(ManagementCard,tmp_order_by[0]), tmp_order_by[1])()).limit(limit).all()
    db.session.close()
